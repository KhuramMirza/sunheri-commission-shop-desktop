...
app.whenReady().then(() => {
  ...
  // Silent Thermal Receipt Printing IPC Listener ('print-receipt')
  ipcMain.on('print-receipt', async (event, data) => {
    const htmlData = typeof data === 'string' ? data : data?.htmlData
    const options = typeof data === 'object' ? data?.options || {} : {}

    // Unique job identifier for tracking job-specific events
    const jobId = options.jobId || `print_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`
    const cancelChannel = `print-cancel-${jobId}`
    const statusChannel = `print-status-${jobId}`

    // Clean up any IPC event listeners attached specifically for this print job
    const cleanupJobListeners = () => {
      ipcMain.removeAllListeners(cancelChannel)
      ipcMain.removeAllListeners(statusChannel)
      ipcMain.removeAllListeners('print-receipt-cancel')
    }

    // Create hidden BrowserWindow with background throttling disabled
    ...
    // Attach temporary cancellation listener for this specific job
